...
- **Music tools**: Level settings ▸ Music ▸ Detect BPM & offset listens to the song (`BeatDetector.cs`: onset
  envelope, autocorrelation over 70–190 BPM, phase search) and sets the tempo and first-beat offset; Tap
  tempo takes it from your clicks. The timeline strip paints the song's waveform behind the bar lines,
  mapped through every speed portal. Sync check is a playtest that pulses on every beat and then marks each
  jump green, amber or red by how far it was from the grid.
- **Mount sound sets**: every mount has its own landing and death sound (`land_<mount>`, `death_<mount>`,
  generated by `Tools/sfx/mount_sounds.py`), falling back to the shared clips.
- **Editor**: group folders (rename, tint in the editor, expand to see the type counts), the Curve tool (a
  Catmull-Rom spline through the path points with objects turned to follow it), Auto-decorate (themed
  scenery on editor layer 9: castle, forest, cave, village, graveyard), a ★ Favourites shelf with starred
  objects and brush presets (rotation, scale, flips, colour and properties), playtest bookmarks (View dock
  and cyan diamonds on the timeline), level thumbnails in the quest list, the campaign road and Files,
  Restore autosave (the last ten versions per level), level packs (several quests in one code), and a
  first-run editor tour that can be replayed from the View dock or Help.
- **Play**: rune effects (dash speed lines, combo counter and screen ripple on orb chains), a coin magnet
  with a chime that climbs in pitch along a streak, and the game keeps running after an unexpected error
  (`CrashGuard.cs` writes a bug report with the last log lines to the captures folder and pauses the run).
- **Deeds**: twenty achievements (`Achievements.cs`) announced with a horn and listed on the title's Deeds
...
renders, ids are unique, share codes, level packs and stamps round-trip, the beat detector finds a synthetic
tempo, and every built-in level runs a few seconds
